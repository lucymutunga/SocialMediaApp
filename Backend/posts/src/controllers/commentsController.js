    ...
    try {
      ...
      if (pool.connected) {
        let results = await pool.query(
          `SELECT * FROM media.comment WHERE comment_id = ${Number(comment_id)}`
        );
        console.log(results);
  ...
  deleteComment: async (req, res) => {
    try {
      ...
      if (pool.connected) {
        ...
        const deleteLikesQuery = `DELETE FROM media.commentslike WHERE comment_id = ${Number(
          comment_id
        )}`;
        await pool.query(deleteLikesQuery);

        // Delete the comment
        const deleteCommentQuery = `DELETE FROM media.comment WHERE comment_id = ${Number(
          comment_id
        )}`;
        const results = await pool.query(deleteCommentQuery);
